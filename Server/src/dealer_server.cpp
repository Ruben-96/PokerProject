...
{
public:
    chat_session(tcp::socket socket, chat_room& room, Game& game_)
        ...
        asio::async_read(socket_,
            ...
            [this, self](std::error_code ec, std::size_t /*length*/)
            {
                if (!ec)
                {
                    ...
                    room_.deliver( game.event_handler(read_msg_.body()) );
                    do_read_header();
                }
                else
                {
...
{
public:
    chat_server(asio::io_context& io_context, const tcp::endpoint& endpoint, Game& game_) : acceptor_(io_context, endpoint), game(game_)
    {
        do_accept();
    }
...
private:
    void do_accept()
    {
        acceptor_.async_accept(
            ...
            if (!ec)
            {
                std::make_shared<chat_session>(std::move(socket), room_, game)->start();
            }
            do_accept();
            }
        );
